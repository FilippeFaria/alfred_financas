...
class _SettingsPageState extends ConsumerState<SettingsPage> {
  ...
  final Map<String, TextEditingController> _controladoresCartao = <String, TextEditingController>{};
  ...
  Future<void> _carregarPreferenciasSms() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Nao foi possivel carregar preferencias de SMS.')),
      );
    } finally {
      if (mounted) {
        setState(() {
  ...
  void _sincronizarControladoresCartao(Map<String, String> mapping) {
    ...
    for (final cartao in alvo) {
      ...
      if (existente == null) {
        _controladoresCartao[cartao] = TextEditingController(text: mapping[cartao] ?? '');
      } else {
        existente.text = mapping[cartao] ?? '';
      }
    }
    final paraRemover = _controladoresCartao.keys.where((key) => !alvo.contains(key)).toList();
  ...
  Future<void> _sincronizarPermissaoSms() async {
    ...
    try {
      final granted = await _notificationChannel.invokeMethod<bool>('isSmsPermissionGranted') ?? false;
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _pedirPermissaoSms() async {
    ...
    try {
      final granted = await _notificationChannel.invokeMethod<bool>('requestSmsPermission') ?? false;
      if (!mounted) return;
      setState(() {
        _smsPermissionGranted = granted;
      });
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Nao foi possivel solicitar permissao de SMS.')),
      );
    }
  }
  ...
  bool _validarMapeamentoLocal(Map<String, String> mapping) {
    ...
    for (final entry in mapping.entries) {
      ...
      if (!RegExp(r'^\d{4}$').hasMatch(valor)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Digite 4 digitos validos para "${entry.key}".')),
        );
        return false;
      }
  ...
  Future<void> _salvarPreferenciasSms() async {
    ...
    try {
      ...
      await _sincronizarConfigNativa();
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Preferencias de SMS salvas com sucesso.')),
      );
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Nao foi possivel salvar preferencias de SMS.')),
      );
    } finally {
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final suportaSmsAndroid = !kIsWeb && defaultTargetPlatform == TargetPlatform.android;
            ...
            child: ListTile(
              ...
              trailing: IconButton(
                ...
                onPressed: () async {
                  ...
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('URL copiada para a area de transferencia.')),
                    );
                  }
                },
                  ...
                  : Column(
                      ...
                      children: [
                        ...
                        SwitchListTile(
                          ...
                          title: const Text('Ativar captura automatica por SMS'),
                          value: _smsEnabled,
                          onChanged: (value) {
                            setState(() {
                              _smsEnabled = value;
                        ...
                        if (_catalogoBancos.isEmpty)
                          const Text('Nenhum banco disponivel.', style: TextStyle(color: Colors.black54))
                        else
                          Wrap(
                            spacing: 8,
                            runSpacing: 8,
                            children: _catalogoBancos
                                .map(
                                  (banco) => FilterChip(
                                    ...
                                    selected: _bancosSelecionados.contains(banco.id),
                                    onSelected: (selected) {
                                      setState(() {
                                        if (selected) {
                        ...
                        if (_catalogoCartoes.isEmpty)
                          const Text('Nenhum cartao disponivel.', style: TextStyle(color: Colors.black54))
                        else
                          ..._catalogoCartoes.map((cartao) {
                            final controller = _controladoresCartao[cartao] ?? TextEditingController();
                            _controladoresCartao.putIfAbsent(cartao, () => controller);
                            return Padding(
